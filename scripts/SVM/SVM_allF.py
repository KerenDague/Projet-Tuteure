...
class L1NumericExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for doc in self.nlp_.pipe(X, disable=["ner"]):
            ...
            verb_positions, subj_positions, obj_positions = [], [], []
            plural_count = 0
    ...
    def transform(self, X):
        ...
        for doc in self.nlp_.pipe(X, disable=["ner"]):
            ...
            for t in tokens:
                if t.dep_ == "auxpass":
                    nb_passif += 1
                elif t.pos_ == "AUX" and t.lemma_ == "être" and any(c.dep_ == "nsubj:pass" for c in t.head.children):
                    nb_passif += 1
            ...
            verbes_conjugues = [
                t for t in tokens
                if t.pos_ in ("VERB", "AUX") and t.morph.get("VerbForm") not in (["Inf"], ["Part"])
            ]
            nb_verbes_conj= max(1, len(verbes_conjugues))
            nb_pron_sujet = sum(1 for t in tokens if t.text.lower() in PRONOMS_SUJETS and t.dep_ == "nsubj")
...
def build_svm_pipeline():
    ...
    vectorizer = ColumnTransformer([
        ...
        ('l1_numeric', L1NumericExtractor(), TEXT_COLUMN),
        ('l1_dep',l1_pipeline_dep,  TEXT_COLUMN),
        ('advanced', AdvancedLinguisticExtractor(), TEXT_COLUMN),
    ], transformer_weights={
        'char_tfidf': 1.0,
        'word_tfidf': 1.0,
        'pos':  0.6,
